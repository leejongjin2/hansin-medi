...
def main(args):
    ...
    if args.random_state == -1:
        args.random_state = random.randint(1,8888)
        print(f"random_state: {args.random_state}")
    if args.n_estimators == -1:
        args.n_estimators = random.randint(1,8888)
    ...
    for task in args.task:
        ...
        rf_run = RandomForestClassifier(n_estimators=args.n_estimators, random_state=args.random_state)
        ...
        print('모델 테스트 세트 정밀도 : {:.3f}'.format(precision_score(y_test, pred, average='micro'))) 
        print('모델 테스트 세트 재현율 : {:.3f}'.format(recall_score(y_test, pred, average='micro')))
        print('모델 F1 Score : {:.3f}'.format(f1_score(y_test, pred, average='micro')))
        

        os.makedirs(os.path.join(args.output_dir, "train"), exist_ok=True)
        file_name = f"{task}_{int2task[task]}_{rf_run.score(X_test, y_test):.3f}.joblib"
